...
if(trim($_POST["action"]) == "addbalance"){
	...
	echo '{"status":"success","url":"https://primearea.biz/buy/'.$caseS["primearea"][trim($_POST["system"])].'"}';exit;
}
...
$arrsse[] = $new_tmp;
for($i=0;$i<count($arrs);$i++)$arrsse[] = $arrs[$i];
...
$st1 = mysql_query("INSERT INTO `questions`(`price`,`category`, `answerer`, `title`, `text`, `answer`, `created`, `changed`, `answered`, `author`, `shown`, `images`) VALUES ('$pricesell','$category','$answerer','$title','$text','$answer','$created','$changed','$answered','$author','$shown','$image')")or die(mysql_error());
