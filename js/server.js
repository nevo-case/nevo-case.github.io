...
$(document).on('click', '.paytype', function(e) {
	...
	function bagSS() {
		if($('#lastWinners')[0] &&  $('#lastWinners')[0].innerHTML == ""){
	...
	function fillCarusel() {
		var a1 = cases[currentCase].filter(function(weapon) { return weapon[2] == 'milspec' }).slice(0).mul(15).shuffle()
		...
		$.ajax({
			...
			success: function(data) {
				if (data.status == 'success') {
					...
					$('#casesCarusel > div:nth-child('+ itemNum +'), #weaponBlock .recweap').removeClass('milspec restricted classified covert rare').addClass(weapon.type)
					
					$('#casesCarusel > div:nth-child('+ itemNum +') .weaponblockinfo span').html((weapon.stattrack ? ' ' : '') + weaponName.replace(' | ', '<br/>'))
					$('#casesCarusel > div:nth-child('+ itemNum +')').find('img').attr('src', getImage(weapon.image, 100, 100))
					...
					var vkTitle = encodeURI('Кейсы World of Tanks - Открывай с выгодой')
					var vkText = (weapon.stattrack ? '' : '') + weaponName
					vkText = encodeURI('Я выиграл ' + vkText)
					var vkImage = getImage(weapon.image, 360, 360)
					vkImage = vkImage.replace("//", 'http://')
					var wItem = $('#casesCarusel > div').outerWidth()
					var a = wItem * (itemNum - 1);
					$('#casesCarusel').animate({ marginLeft: -1 * Math.rand(a + (wItem/3), a + (wItem *0.66)) }, {
						duration: 10000,
						easing: 'swing',
						//easing: 'easeInSine',
						start: function() {
						...
						complete: function() {
							...
							$('.shareBtn').html(VK.Share.button({
								...
								type: 'custom',
								text: '<img src="/css/vk_icon.png" /><span>Раскажи друзьям в ВК</span>'
							}))
								
							
							$("#sellBlock").hide()
							$("#afterDemoBlock1").hide()
	...
	$('#CaseFree').click(function() {
		...
		var weaponName = $('#casesCarusel > div:nth-child('+ itemNum +') .weaponblockinfo span').text();
		var image = $('#casesCarusel > div:nth-child('+ itemNum +')').find('img').attr('src');
		console.log(image);

		$('#weaponBlock .recweaptitle .name').text(weaponName)
		$('#weaponBlock .recweap img').attr('src', image)
